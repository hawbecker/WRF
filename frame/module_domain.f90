...
ENDIF
IF ( ASSOCIATED( grid%coszr ) ) THEN 
  DEALLOCATE(grid%coszr,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19749,&
'frame/module_domain.f: Failed to deallocate grid%coszr. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%rswdir_htop_vis ) ) THEN 
  DEALLOCATE(grid%rswdir_htop_vis,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19756,&
'frame/module_domain.f: Failed to deallocate grid%rswdir_htop_vis. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%rswdir_htop_red ) ) THEN 
  DEALLOCATE(grid%rswdir_htop_red,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19763,&
'frame/module_domain.f: Failed to deallocate grid%rswdir_htop_red. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%rswdif_htop_vis ) ) THEN 
  DEALLOCATE(grid%rswdif_htop_vis,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19770,&
'frame/module_domain.f: Failed to deallocate grid%rswdif_htop_vis. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%rswdif_htop_red ) ) THEN 
  DEALLOCATE(grid%rswdif_htop_red,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19777,&
'frame/module_domain.f: Failed to deallocate grid%rswdif_htop_red. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%rlwd_htop ) ) THEN 
  DEALLOCATE(grid%rlwd_htop,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19784,&
'frame/module_domain.f: Failed to deallocate grid%rlwd_htop. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_tlai ) ) THEN 
  DEALLOCATE(grid%mcm_tlai,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19791,&
'frame/module_domain.f: Failed to deallocate grid%mcm_tlai. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_sw ) ) THEN 
  DEALLOCATE(grid%mcm_sw,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19798,&
'frame/module_domain.f: Failed to deallocate grid%mcm_sw. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_lw ) ) THEN 
  DEALLOCATE(grid%mcm_lw,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19805,&
'frame/module_domain.f: Failed to deallocate grid%mcm_lw. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_sh ) ) THEN 
  DEALLOCATE(grid%mcm_sh,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19812,&
'frame/module_domain.f: Failed to deallocate grid%mcm_sh. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_le ) ) THEN 
  DEALLOCATE(grid%mcm_le,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19819,&
'frame/module_domain.f: Failed to deallocate grid%mcm_le. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_co2_tend ) ) THEN 
  DEALLOCATE(grid%mcm_co2_tend,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19826,&
'frame/module_domain.f: Failed to deallocate grid%mcm_co2_tend. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_qv_tend ) ) THEN 
  DEALLOCATE(grid%mcm_qv_tend,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19833,&
'frame/module_domain.f: Failed to deallocate grid%mcm_qv_tend. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_t_tend ) ) THEN 
  DEALLOCATE(grid%mcm_t_tend,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19840,&
'frame/module_domain.f: Failed to deallocate grid%mcm_t_tend. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_q_source ) ) THEN 
  DEALLOCATE(grid%mcm_q_source,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19847,&
'frame/module_domain.f: Failed to deallocate grid%mcm_q_source. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_co2_source ) ) THEN 
  DEALLOCATE(grid%mcm_co2_source,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19854,&
'frame/module_domain.f: Failed to deallocate grid%mcm_co2_source. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_le_grnd ) ) THEN 
  DEALLOCATE(grid%mcm_le_grnd,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19861,&
'frame/module_domain.f: Failed to deallocate grid%mcm_le_grnd. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_sh_grnd ) ) THEN 
  DEALLOCATE(grid%mcm_sh_grnd,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19868,&
'frame/module_domain.f: Failed to deallocate grid%mcm_sh_grnd. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_sw_in ) ) THEN 
  DEALLOCATE(grid%mcm_sw_in,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19875,&
'frame/module_domain.f: Failed to deallocate grid%mcm_sw_in. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_sw_out ) ) THEN 
  DEALLOCATE(grid%mcm_sw_out,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19882,&
'frame/module_domain.f: Failed to deallocate grid%mcm_sw_out. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_lw_in ) ) THEN 
  DEALLOCATE(grid%mcm_lw_in,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19889,&
'frame/module_domain.f: Failed to deallocate grid%mcm_lw_in. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_lw_out ) ) THEN 
  DEALLOCATE(grid%mcm_lw_out,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19896,&
'frame/module_domain.f: Failed to deallocate grid%mcm_lw_out. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_tau13_t ) ) THEN 
  DEALLOCATE(grid%mcm_tau13_t,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19903,&
'frame/module_domain.f: Failed to deallocate grid%mcm_tau13_t. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_tau13_q ) ) THEN 
  DEALLOCATE(grid%mcm_tau13_q,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19910,&
'frame/module_domain.f: Failed to deallocate grid%mcm_tau13_q. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%mcm_tau13_c ) ) THEN 
  DEALLOCATE(grid%mcm_tau13_c,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19917,&
'frame/module_domain.f: Failed to deallocate grid%mcm_tau13_c. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%www ) ) THEN 
  DEALLOCATE(grid%www,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19924,&
'frame/module_domain.f: Failed to deallocate grid%www. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%cd_pan ) ) THEN 
  DEALLOCATE(grid%cd_pan,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19931,&
'frame/module_domain.f: Failed to deallocate grid%cd_pan. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%lad_3d ) ) THEN 
  DEALLOCATE(grid%lad_3d,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19938,&
'frame/module_domain.f: Failed to deallocate grid%lad_3d. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%canopyz_2d ) ) THEN 
  DEALLOCATE(grid%canopyz_2d,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19945,&
'frame/module_domain.f: Failed to deallocate grid%canopyz_2d. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%lad_z_3d ) ) THEN 
  DEALLOCATE(grid%lad_z_3d,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19952,&
'frame/module_domain.f: Failed to deallocate grid%lad_z_3d. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%pert_t ) ) THEN 
  DEALLOCATE(grid%pert_t,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19959,&
'frame/module_domain.f: Failed to deallocate grid%pert_t. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%prttms ) ) THEN 
  DEALLOCATE(grid%prttms,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19966,&
'frame/module_domain.f: Failed to deallocate grid%prttms. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%prtdt ) ) THEN 
  DEALLOCATE(grid%prtdt,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19973,&
'frame/module_domain.f: Failed to deallocate grid%prtdt. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%prtk ) ) THEN 
  DEALLOCATE(grid%prtk,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19980,&
'frame/module_domain.f: Failed to deallocate grid%prtk. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%prtseed ) ) THEN 
  DEALLOCATE(grid%prtseed,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19987,&
'frame/module_domain.f: Failed to deallocate grid%prtseed. ')
 endif
ENDIF
IF ( ASSOCIATED( grid%m_pblh ) ) THEN 
  DEALLOCATE(grid%m_pblh,STAT=ierr)
 if (ierr.ne.0) then
 CALL wrf_error_fatal3("<stdin>",19994,&
'frame/module_domain.f: Failed to deallocate grid%m_pblh. ')
 endif
ENDIF
   ...
   END SUBROUTINE init_module_domain
      ...
      SUBROUTINE domain_clock_get( grid, current_time,                &
        ...
        IF ( PRESENT( currentDayOfYearReal ) ) THEN
          ...
          CALL WRFU_TimeGet( lcl_currtime, dayOfYear_r8=currentDayOfYearR8, &
                             rc=rc )
          IF ( rc /= WRFU_SUCCESS ) THEN
            CALL wrf_error_fatal3("<stdin>",20546,&
      ...
      SUBROUTINE domain_clock_create( grid, StartTime, &
        ...
        grid%domain_clock = WRFU_ClockCreate( TimeStep= TimeStep,  &
                                              ...
                                              rc=rc )
        IF ( rc /= WRFU_SUCCESS ) THEN
          CALL wrf_error_fatal3("<stdin>",20635,&
      ...
      SUBROUTINE domain_alarm_create( grid, alarm_id, interval, &
        ...
        ELSE IF ( (       PRESENT( begin_time ) ) .AND. &
           ...
           all_args = .TRUE.
        ELSE
           CALL wrf_error_fatal3("<stdin>",20682,&
        ...
        ENDIF
        IF ( rc /= WRFU_SUCCESS ) THEN
          CALL wrf_error_fatal3("<stdin>",20707,&
      ...
      SUBROUTINE domain_clock_set( grid, current_timestr, &
        ...
        IF ( PRESENT( time_step_seconds ) ) THEN
          ...
          CALL WRFU_ClockSet ( grid%domain_clock,        &
                               ...
                               rc=rc )
          IF ( rc /= WRFU_SUCCESS ) THEN
            CALL wrf_error_fatal3("<stdin>",20767,&
