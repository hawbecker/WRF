...
SUBROUTINE med_add_config_info_to_grid ( grid )
 ...
 grid % mcm_opt                    = model_config_rec % mcm_opt (grid%id)
 grid % mcm_link_rad               = model_config_rec % mcm_link_rad (grid%id)
 grid % mcm_use_3d_lad             = model_config_rec % mcm_use_3d_lad (grid%id)
 grid % mcm_model                  = model_config_rec % mcm_model (grid%id)
 grid % scpm_jdm_opt               = model_config_rec % scpm_jdm_opt (grid%id)
 grid % scpm_nb                    = model_config_rec % scpm_nb (grid%id)
 grid % scpm_sb                    = model_config_rec % scpm_sb (grid%id)
 grid % scpm_eb                    = model_config_rec % scpm_eb (grid%id)
 grid % scpm_wb                    = model_config_rec % scpm_wb (grid%id)
 grid % m_pblh_opt                 = model_config_rec % m_pblh_opt (grid%id)
